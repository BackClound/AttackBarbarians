...
| `SaveManager` | **是** | `GameSystems` 根或子物体 | `ServiceLocator.Get<SaveManager>()` |
...
`ConfigManager.Initialize` → `SaveManager.Initialize`（含 Load）→ `EventBus.Initialize` → `PoolManager.Initialize` → `GameManager.Initialize` → `GameFlowManager.Initialize` →（若配置允许）`GameManager.StartGame`

游戏状态机详见 **`docs/game_state_system.md`**。存档详见 **`docs/save_system.md`**。
...
## v2 补充（Player 访问与 Bootstrap 顺序）

### PlayerSceneAccess（不挂载）
- 路径：`Assets/Scripts/Utils/PlayerSceneAccess.cs`
- 用途：Talent/Equipment/Experience/Upgrade 等系统获取 `PlayerController`，**禁止** `FindObjectOfType<Player*>`。
- 获取方式：静态方法 `TryGetController` / `TryGetSkillSystem` / `WithReadyController`。

### Bootstrap 初始化顺序（v2 对齐）
```
Config → Save → Talent/Equipment → EventBus → Pool
→ GameManager/GameFlow → RunTracker/Settlement → Experience
→ Upgrade/RandomReward → Damage/Collision/Projectile
→ SkillUnlock/Content → Map/GameplayEvent
→ EnemySpawner → Wave → BossStats
→ GameplayEventDebugBridge（最后注册到 ServiceLocator）
```

### Player 物体组件清单（v2 推荐）
```
Player [Player, PlayerController, Player_Health, PlayerSkillManager,
        SkillManager, BuffManager, AutoAttackController, PlayerCombatBridge]
```
